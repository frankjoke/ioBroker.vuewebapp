...
function prepareTooltips() {
  if (isMaterialize) {
    ...
    buttonsText +=
      '   <a class="btn-floating btn-small waves-effect waves-light adapter-config-load" title="' +
      _("loadConfig") +
      '">' +
    ...
    buttonsText +=
      '   <a class="btn-floating btn-small waves-effect waves-light adapter-config-save" title="' +
      _("saveConfig") +
      '">' +
    ...
    if (buttonsText) {
      ...
      if ($logo.length) {
        $(
          '<div class="col s6 help-link">' + buttonsText + "</div>"
        ).insertAfter($logo);
      }
    ...
    $(".value").each(function() {
      ...
      if (systemDictionary["tooltip_" + id]) {
        tooltip =
          systemDictionary["tooltip_" + id][systemLang] ||
          systemDictionary["tooltip_" + id].en;
      }
      ...
      if (link && common) {
        if (link === true) {
          if (common.readme) {
            ...
            link =
              "https://github.com/ioBroker/ioBroker." + common.name + "#" + id;
          }
        }
        if (!link.match("^https?://")) {
          if (common.readme) {
      ...
      if (link) {
        $(
          '<a class="tooltip" href="' +
            ...
            '" target="_blank"><i class="material-icons tooltip">live_help</i></a>'
        ).insertBefore($this);
      } else if (tooltip) {
        $(
          '<i class="material-icons tooltip" title="' +
            tooltip +
            '">help_outline</i>'
        ).insertBefore($this);
      }
    ...
    $(".admin-icon").each(function() {
      ...
      if (link && common) {
        ...
        icon +=
          '<img class="admin-tooltip-icon" title="' +
          tooltip +
          '" src="../../img/info.png"/>';
      }
      ...
      if (systemDictionary["info_" + id]) {
        $(this).html(
          '<span class="admin-tooltip-text">' +
            (systemDictionary["info_" + id][systemLang] ||
              systemDictionary["info_" + id].en) +
            "</span>"
        );
      }
    });
  ...
  if (typeof buttons === "function") {
    ...
    for (var b = 0; b < buttons.length; b++) {
      buttons[b] = {
        ...
        click: function(e) {
          var id = parseInt(
            e.currentTarget.id.substring("dialog-confirm-button-".length),
            10
          );
          var cb = $(this).data("callback");
          $(this).data("callback", null);
...
function getIPs(host, callback) {
  ...
  socket.emit("getHostByIp", host || common.host, function(ip, _host) {
    if (_host) {
      ...
      var IPs4 = [
        {
          name: "[IPv4] 0.0.0.0 - " + _("Listen on all IPs"),
          address: "0.0.0.0",
          family: "ipv4"
        }
      ];
      ...
      if (host.native.hardware && host.native.hardware.networkInterfaces) {
        for (var eth in host.native.hardware.networkInterfaces) {
          ...
          for (
            var num = 0;
            num < host.native.hardware.networkInterfaces[eth].length;
            num++
          ) {
            if (
              host.native.hardware.networkInterfaces[eth][num].family !== "IPv6"
            ) {
              IPs4.push({
                name:
                  "[" +
...
function sendTo(_adapter_instance, command, message, callback) {
  socket.emit(
    "sendTo",
    _adapter_instance || adapter + "." + instance,
    command,
    message,
    callback
  );
...
function getInterfaces(onlyNames, callback) {
  ...
  socket.emit("sendToHost", common.host, "getInterfaces", null, function(
    result
  ) {
...
function getIsAdapterAlive(_adapter, callback) {
  ...
  getState(
    "system.adapter." + (_adapter || adapter) + "." + instance + ".alive",
    function(err, obj) {
      if (!obj || !obj.val) {
        callback(false);
      } else {
...
function _editInitButtons($grid, tabId, objId) {
  ...
  $("." + tabId + "-edit-submit" + search)
    ...
    .on("click", function() {
      ...
      $(
        "." + tabId + "-cancel-submit[data-" + tabId + '-id="' + id + '"]'
      ).show();
  ...
  $("." + tabId + "-ok-submit" + search)
    ...
    .on("click", function() {
      ...
      if ($grid[0]._onChange)
        $grid[0]._onChange(
          "changed",
          $grid.jqGrid("getRowData", tabId + "_" + id)
        );
    })
    .css({ height: "18px", width: "22px" });
...
function _editTable(tabId, cols, values, rooms, top, onChange) {
    ...
    if (cols[i] === "room") {
      ...
      for (room in rooms) {
        _obj.searchoptions.value +=
          ";" +
          _(translateName(rooms[room].common.name)) +
          ":" +
          _(translateName(rooms[room].common.name));
      }
    }
  ...
  $grid
    .jqGrid({
      ...
      ondblClickRow: function(rowid) {
        ...
        $("." + tabId + "-delete-submit").hide();
        $(
          "." + tabId + "-ok-submit[data-" + tabId + '-id="' + id + '"]'
        ).show();
        $(
          "." + tabId + "-cancel-submit[data-" + tabId + '-id="' + id + '"]'
        ).show();
  ...
  $(window).on("resize", function() {
    $grid
      .setGridHeight($(this).height() - top)
      .setGridWidth($(this).width() - 10);
  });
  ...
  for (var enumId in enums) {
    ...
    if (enums[enumId] && enums[enumId].common && enums[enumId].common.name) {
      if (typeof enums[enumId].common.name === "object") {
        ...
        if (
          enums[enumId].common.name &&
          enums[enumId].common.name.toLowerCase() === name
        )
          return enumId;
      }
    }
    if (enums[enumId] && enums[enumId].name) {
      if (typeof enums[enumId].name === "object") {
        for (var lang in enums[enumId].name) {
          if (
            enums[enumId].name.hasOwnProperty(lang) &&
            enums[enumId].name[lang].toLowerCase() === name
          ) {
            return enumId;
          }
        }
...
function getTableResult(tabId, cols) {
  ...
  for (var j = 0; j < $grid[0]._edited.length; j++) {
    $grid.jqGrid("saveRow", tabId + "_" + $grid[0]._edited[j], {
      url: "clientArray"
    });
  }
...
function values2table(divId, values, onChange, onReady, maxRaw) {
  ...
  if (values) {
      ...
      getEnums("rooms", function(err, list) {
        ...
        for (var l = 0; l < nnames.length; l++) {
          ...
          if (typeof result[nnames[l]] === "object") {
            result[nnames[l]] =
              result[nnames[l]][systemLang] || result[nnames[l]].en;
          }
        }
    ...
    if (
      ...
      $table.find('th[data-name="func"]').length
    ) {
      ...
      for (var i = 0; i < names.length; i++) {
        ...
        if (names[i]) {
          ...
          if (names[i].name === "_index") {
            ...
            line +=
              ...
              '" data-old-value="' +
              (values[v][names[i].name] === undefined
                ? ""
                : values[v][names[i].name]) +
              '"/>';
            if (isMaterialize) {
              line += "<span></span>";
            }
        ...
        if (buttons[i]) {
          ...
          for (var b = 0; b < buttons[i].length; b++) {
            if (
              (!v && buttons[i][b] === "up") ||
              (v === values.length - 1 && buttons[i][b] === "down")
            ) {
              if (isMaterialize) {
                line +=
                  '<a data-command="' +
      ...
      if (command === "copy") {
        ...
        $(this).on("click", function() {
          ...
          if (typeof editLine === "function") {
            setTimeout(function() {
              editLine(id, JSON.parse(JSON.stringify(values[id])), function(
                err,
                id,
                newValues
              ) {
                if (!err) {
                  if (
                    JSON.stringify(values[id]) !== JSON.stringify(newValues)
                  ) {
                    onChange && onChange();
                    values[id] = newValues;
                    values2table(divId, values, onChange, onReady);
      ...
      .on("change.adaptersettings", function() {
        if ($(this).attr("type") === "checkbox") {
          if (
            $(this)
              .prop("checked")
              .toString() !== $(this).data("old-value")
          )
            onChange();
          values[$(this).data("index")][$(this).data("name")] = $(this).prop(
            "checked"
          );
        } else {
...
function encrypt(key, value) {
  ...
  for (let i = 0; i < value.length; i++) {
    result += String.fromCharCode(
      key[i % key.length].charCodeAt(0) ^ value.charCodeAt(i)
    );
  }
  return result;
}
